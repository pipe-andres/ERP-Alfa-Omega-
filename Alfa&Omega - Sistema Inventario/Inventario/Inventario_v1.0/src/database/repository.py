...
def get_product_by_code(codigo: str) -> Optional[Tuple]:
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("SELECT codigo, nombre, categoria, precio, cantidad, avg_cost FROM productos WHERE codigo = ?", (codigo,))
        row = cur.fetchone()
...
def insert_product(data: Dict[str, Any]) -> None:
    with get_connection() as conn:
        ...
        cur.execute("""
            ...
            VALUES (?,?,?,?,?,?)
        """, (data['codigo'], data['nombre'], data.get('categoria',''), float(data.get('precio',0.0)), int(data.get('cantidad',0)), float(data.get('avg_cost', data.get('precio',0.0)))))
        conn.commit()
...
# Kardex helpers
# ---------------------------
def record_kardex_move(product_code: str, move_type: str, qty: float, unit_cost: float, total_cost: float, balance_qty: float, balance_cost: float, balance_total: float, date: str, warehouse_id: int = None, ref_type: str = None, ref_id: int = None) -> int:
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
        """, (product_code, move_type, float(qty), unit_cost, total_cost, float(balance_qty), balance_cost, balance_total, date, warehouse_id, ref_type, ref_id))
        conn.commit()
