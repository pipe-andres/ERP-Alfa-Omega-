...
def has_perm(user: Dict, perm_or_role: str) -> bool:
    """
    Si el usuario tiene el rol ADMIN, siempre True.
    Si 'perm_or_role' está en roles del usuario, True.
    (Simplificado para tu GUI actual.)
    """
...
def _apply_roles(cur, user_id: int, roles: List[str]) -> None:
    ...
    for r in roles:
        ...
        if not rr:
            ...
            cur.execute("INSERT INTO roles(name) VALUES(?) ON CONFLICT(name) DO NOTHING RETURNING id", (r,))
            result = cur.fetchone()
