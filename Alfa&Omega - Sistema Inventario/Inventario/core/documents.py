...
def ensure_schema() -> None:
    ...
    """
    # Si usamos Postgres en producción, Alembic maneja el esquema
...
def get_next_number(doc_type: str, series: str) -> str:
    """
    Toma y avanza el numerador para (doc_type, series).
    ...
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("SELECT id, prefix, next_no FROM doc_series WHERE doc_type=? AND series=?", (doc_type, series))
        row = cur.fetchone()
        if not row:
            # si no existe, la creamos con prefix = series + "-"
            cur.execute("INSERT INTO doc_series (doc_type, series, prefix, next_no) VALUES (?,?,?,1) RETURNING id",
