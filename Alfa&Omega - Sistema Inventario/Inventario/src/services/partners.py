...
def get_supplier_history(partner_id: int, limit: int = 20) -> List[Tuple]:
    """
    Obtiene las últimas compras al proveedor.
    Retorna lista de tuples: (fecha, numero, total, estado)
    Solo documentos con tipo='PURCHASE' y partner_id dado.
    """
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("""
            SELECT 
                d.fecha,
                d.numero,
                COALESCE(SUM(dl.qty * dl.unit_price), 0) as total,
                'Comprado'  -- estado fijo por ahora
            FROM documents d
            LEFT JOIN document_lines dl ON dl.doc_id = d.id
            WHERE d.tipo = 'PURCHASE' AND d.partner_id = ?
            GROUP BY d.id, d.fecha, d.numero
...
def get_supplier_orders(partner_id: int) -> List[Tuple]:
    """
    Obtiene las órdenes de compra pendientes de este proveedor.
    Retorna lista de tuples: (fecha, numero, total, estado)
    Filtra documentos tipo='PURCHASE' con estado pendiente.
    """
    with get_connection() as conn:
        cur = conn.cursor()
        cur.execute("""
            SELECT 
                ...
                d.numero,
                COALESCE(SUM(dl.qty * dl.unit_price), 0) as total,
                'Pendiente'  -- estado: pendiente/recibida/parcial (Fase 2 agregar columna estado)
            FROM documents d
            LEFT JOIN document_lines dl ON dl.doc_id = d.id
            WHERE d.tipo = 'PURCHASE' AND d.partner_id = ?
            GROUP BY d.id, d.fecha, d.numero
