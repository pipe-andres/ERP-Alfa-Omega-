...
def ensure_schema() -> None:
    ...
    if DB_ENGINE == 'json':
        ...
        # Ensure the series table exists as a JSON file
        json_store.save_table('doc_series', json_store.load_table('doc_series'))
        return

    with get_connection() as conn:
        cur = conn.cursor()

        # Tabla de numeradores (solo SQLite llega aquí)
        cur.execute("""
...
def seed_default_series() -> None:
    ...
    if DB_ENGINE == 'json':
        from src.database import json_store
        series_list = json_store.load_table('doc_series')
        for t, s in [("PURCHASE", "C01"), ("SALE", "V01")]:
            if not json_store.find_one('doc_series', lambda r: r.get('doc_type') == t and r.get('series') == s):
                json_store.insert('doc_series', {
                    'doc_type': t,
                    'series': s,
                    'prefix': s + "-",
                    'next_no': 1
...
def get_next_number(doc_type: str, series: str) -> str:
    ...
    Compatible con SQLite, Postgres y JSON.
    """
    ...
    try:
        if DB_ENGINE == 'json':
            ...
            for i, row in enumerate(series_list):
                if row.get('doc_type') == doc_type and row.get('series') == series:
                    found = row
                    break
            ...
            prefix = found.get('prefix', series + "-")
            next_no = int(found.get('next_no', 1))
            numero_impreso = f"{prefix}{next_no:06d}"
            found['next_no'] = next_no + 1
            json_store.save_table('doc_series', series_list)
        ...
        with get_connection() as conn:
            ...
            if not row:
                # Crear nueva serie (usa ? que CursorWrapper traduce a %s en Postgres)
                cur.execute("INSERT INTO doc_series (doc_type, series, prefix, next_no) VALUES (?,?,?,1)",
                           (doc_type, series, series + "-"))
                cur.execute("SELECT id, prefix, next_no FROM doc_series WHERE doc_type=? AND series=?",
                           (doc_type, series))
                row = cur.fetchone()
                ds_id, prefix, next_no = row[0], row[1] or (series + "-"), 1
            else:
        ...
        try:
            ...
            log_error(e, context={"operation": "get_next_number", "doc_type": doc_type, "series": series})
        except Exception:
            pass
        raise
